...
func (db *DB) Stat() *Stat {
	...
	if err != nil {
		panic(fmt.Sprintf("falied to get dir size: %v", err))
	}
	return &Stat{
		uint(db.index.Size()),
		dataFiles,
...
// 获取所有的数据，并执行用户指定的操作函数返回 false 时终止遍历
...
func checkOptions(options Options) error {
	...
	if options.DataFileSize <= 0 {
		return errors.New("database DataFileSize must be greater than 0")
	}
	if options.DataFileMergeRation < 0 || options.DataFileMergeRation > 1 {
		return errors.New("database DataFileMergeRation must be between 0 and 1")
	}
	return nil
}
...
func (db *DB) loadIndexFromDataFiles() error {
	...
	for i, fid := range db.fileIds {
		...
		//如果比最近未参与merge的文件id 更小，则说明已经从hint文件中加载索引了
